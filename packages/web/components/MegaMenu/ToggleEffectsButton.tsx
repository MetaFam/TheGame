import { Box, Button, HStack, Icon, Tooltip, usePrefersReducedMotion, useToast } from '@metafam/ds';
 ...
 */
const ToggleEffectsButton: FC<ToggleEffectsButtonProps> = ({ hasLabel = false }) => {
  const { pathname } = useRouter();
  const isLandingPage = pathname === '/';
  const toast = useToast();
  const prefersReducedMotion = usePrefersReducedMotion();
  const savedEffectsPreference = get('EffectsPreference') === 'enabled';
  const [effectsEnabled, setEffectsEnabled] = useState<boolean>(savedEffectsPreference  );
  ...
  return (
    <HStack spacing={0} position={{base: 'absolute', xl: 'relative'}} bottom={{base: 4, xl: 'unset'}} left={{base: 0, xl: 'unset'}}>
      ...
      {hasLabel ? <Box as="span" fontSize="small">Turn {`${effectsEnabled ? 'off' : 'on'}`} effects</Box> : null}
    </HStack>
  );
};
