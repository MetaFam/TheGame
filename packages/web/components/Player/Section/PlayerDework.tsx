import { Box, ExternalLinkIcon, Image, Spinner, Text, VStack, Wrap, WrapItem } from '@metafam/ds';
...
export const DeworkSectionWrapper: React.FC = ({ children }: { children: React.ReactNode }) => (
  <VStack display="inline-flex" flex="0 0 50%" textAlign="left" alignItems="left" mb={3} gap={0}>
    {children}
  </VStack>
);
...
export const PlayerDework: React.FC<Props> = ({
  ...
  return (
    <ProfileSection title="Dework Profile" {...{ isOwnProfile, editing }}>
      <Wrap>
        <WrapItem position="relative" mb={0} display={'flex'} flexFlow={'row wrap'} width="full">
          {deworkData && processedData && !loading ? (
            ...
            <DeworkSectionWrapper>
              ...
              {processedData?.tagGrouping.length > 0 ? (
                processedData?.tagGrouping.map((tag: Record<string, number>, i: number) => {
                  const total: number = processedData?.tagGrouping.length;
                  if (i < 3) {
                    return (
                      <>
                      <Text key={generateUUID()} textTransform="capitalize"  fontSize={{base: 'sm', xl: 'md'}}>
                        <Box as="span" textOverflow="ellipsis" >{tag.name}</Box> ({tag.count})
                      </Text>
                      {i === 2 && <Box as="i" fontSize="sm">{`+${total - 3} other`}</Box>}
                      </>
                    );
                  }
                  return undefined;
            ...
            <DeworkSectionWrapper>
              ...
              {processedData?.uniqueOrganisations.length > 0 ? (
                processedData?.uniqueOrganisations.map((org: Record<string, string>, i: number) => {
                  const total: number = processedData?.uniqueOrganisations.length;
                  if (i < 3) {
                    return (
                      <>
                        <Text key={generateUUID()} fontSize={{base: 'sm', xl: 'md'}}>
                          {org.name}
                        </Text>
          ...
          <Box display="flex" justifyContent="center" alignItems="center" height="100%" width="full">
      ...
      <Image
        ...
        alt="Dework logo"
        />
        {deworkData && (<MetaLink href={`https://app.dework.xyz/profile/${deworkData.address}`} fontWeight={500} display="inline-flex" alignItems="center" isExternal>
