  ...
  return (
    <Modal {...{ isOpen, onClose }}>
      ...
      <ModalContent>
        ...
        <ModalHeader>
          {(!showSearchForm && currentGuilds.length > 0 && !showGuildForm) && 'New Guild Added!'}
          {(showGuildForm || (!currentGuilds.length && !showSearchForm)) && 'Create New Guild'}
          {(showSearchForm || (!currentGuilds.length && !showGuildForm)) && 'Search for a Guild'}
          <Text
            fontStyle="italic"
            color="gray.400"
            textAlign="center"
            ...
            mb={10}
          >
            {(currentGuilds.length > 0 && !showSearchForm && !showGuildForm) && 'Your membership has been added to your guild info.'}
        ...
        <ModalBody p={[0, 6]}>
          ...
          {!showGuildForm && (
            <Button
              ...
              ml={{ base: '20%', lg: '40%' }}
              leftIcon={<AddIcon />}
              onClick={() => { setShowGuildForm(true); setShowSearchForm(false) }}
            >
