...
export const getLatestEthAddress = (identity: SCIdentity): string | null => {
  const ethAddresses = identity.aliases.filter((alias: Alias) => {
    ...
    return parts.includes('ethereum') && isAddress(alias.description);
  });
  if (ethAddresses.length > 0) {
    // this assumes that the fetched data is in chronological order,
    // which should be the case for the SC Ledger
