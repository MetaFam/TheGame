...
import parseSignup from '../parser/signup';
  ...
  try {
    ...
    fetch(`${GITHUB_API_URL}/repos/${environment('GITHUB_FILE_PATH')}`, {
      ...
      .then((body) => {
        ...
        log(`fetched file with sha ${fileSha} for user ${username}`);
        ...
        const { identities } = decodedContent[1];

        // First, check if the identity already exists.
        const existingIdentityIndex = identities.findIndex((identity) =>
        ...
        if (existingIdentityIndex !== -1) {
          ...
          decodedContent[1].identities[existingIdentityIndex].aliases.push(
            ...platformsToAdd,
          );
        } else {
